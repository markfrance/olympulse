...
contract OlympulseBondDepository is Ownable {
    ...
    IERC20 public immutable Olympulse; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond (PLS)
    ITreasury public immutable treasury; // mints Olympulse when receives principle
    ...
    struct Bond {
        uint payout; // Olympulse remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Olympulse ( underflow protection )
        ...
        uint balanceBefore = Olympulse.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Olympulse.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Olympulse.balanceOf(address(this)), "Not enough Olympulse to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Olympulse.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Olympulse.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Olympulse.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Olympulse.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Olympulse supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Olympulse.totalSupply();
    ...
    /**
     *  @notice calculate amount of Olympulse available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Olympulse) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Olympulse, "NAT" );
