...
interface IOlympulseERC20 is IERC20Mintable, IERC20 {
...
contract OlympulseTreasury is Ownable {
    ...
    IOlympulseERC20 public immutable Olympulse;
    ...
    constructor (
        ...
        Olympulse = IOlympulseERC20(_Olympulse);
    ...
    /**
        @notice allow approved address to deposit an asset for Olympulse
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Olympulse needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Olympulse.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Olympulse for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Olympulse.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = Three.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Olympulse
        @param _amount uint
     */
    function repayDebtWithOlympulse( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Olympulse.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Olympulse), _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        Olympulse.mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Olympulse.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Olympulse valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Olympulse decimals
            value_ = _amount.mul( 10 ** Olympulse.decimals() ).div( 10 ** IERC20( _token ).decimals() );
